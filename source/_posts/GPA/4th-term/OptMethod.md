...
- 选择 `6 * 2'`
    ...
    - [ ] 拉格朗日函数中等式乘子的性质（ch1.3）（不等式对应的乘子 $\mu\ge0$​，等式约束对应的乘子 $\lambda$ 是任意的）
...
- 计算 `68'`
    ...
    - [ ] 等式二次罚函数法（ch7）（一个约束函数，一个等式约束条件，共 4 问）
        1. 用拉格朗日定理求最优解 $x^*$ 和拉格朗日乘子 $\lambda^*$
    ...
    - [x] ~~有效集方法解不等式约束的二次规划问题（ch6）（小概率考到，考到就gg，因为不会）~~
