...
由于本书介绍的最优化求解方法一般只适用于求解**局部最优解**，那么如何确定**全局最优解**呢？以及如何确定**唯一的全局最优解**呢？本目揭晓答案：$\text{局部最优解}+(\text{严格})\text{凸目标函数}+\text{凸可行域}=(\text{唯一})\text{全局最优解}$
...
![实例](https://dwj-oss.oss-cn-nanjing.aliyuncs.com/images/202404171018860.jpg)
...
- 选择 `6 * 2'`
    ...
    - 凸规划问题的定义和性质（ch1.2）
    - 求二元函数的极值（ch1.3）（先利用一阶必要条件求驻点，然后利用海塞矩阵的正定性判定极值点。特征值之和为海塞矩阵的迹，特征值之积为海塞矩阵的行列式，若海塞矩阵正定则对应极小，负定对应极大。答案：一个极大一个极小，另两个什么都不是）
    - 拉格朗日函数中等式乘子的性质（ch1.3）（等式对应的乘子 $\lambda>0$​）
    - 海塞矩阵正定负定和极值的关系（ch1.3）（显然的）
- 填空 `5 * 2'`
  
    - 求解下降方向（ch1.3）
    - 线性规划的基本概念（ch2.1）
    - 拟牛顿法的近似海塞矩阵公式（ch4.4）（见书4.4.9和4.4.10）
- 证明 `1 * 10'`
    - 证明高维凸规划问题（ch1.2）（证明可行域是凸集、目标函数是凸函数即可）
- 计算 `68'`
    - 计算点到超平面的距离，转化为等式约束的最优化问题（ch1.3）（目标函数为超平面中的一点 $x_a$ 到指定点 $x_b$ 的距离，约束条件为 $x_a$ 在超平面上。求解方法就是构造一个拉格朗日函数，然后利用一阶必要条件即可求解）
    - 线性规划问题中求对偶问题表达式（ch2.3.1）
    - 线性规划问题中已知原问题最优解，求解对偶问题最优解（ch2.3.2）（利用互补松弛定理、原问题和对偶问题最优解相等）
    - 0.618法求精确步长（ch3.2）
    - 最速下降法、牛顿法求解方法及其优缺点、共轭梯度法的优缺点（ch4）（直接给步长）
    - 有约束的二次规划，约束条件中既有等式也有不等式（ch6）
    - 等式二次罚函数法（ch7）（一个约束函数，一个等式约束条件）
        - 用拉格朗日定理求最优解 $x^*$ 和拉格朗日乘子 $\lambda^*$
    
        - 写出二次罚函数表达式 $P_E(x,\sigma)$，讨论罚因子 $\sigma$ 取何值时可以使得海塞矩阵 $\nabla^2P_E(x,\sigma)$​ [正定](https://baike.baidu.com/item/%E6%AD%A3%E5%AE%9A%E7%9F%A9%E9%98%B5/11030459?fr=ge_ala#5)（说直接写让行列式大于零的计算结果即可，樂）
    
        - 求最优解 $x^*$（直接计算一阶梯度 $\nabla$ 并消去 $\sigma$ 即可）
    
        - 当 $\sigma \to \infty$​ 时求最优解，并判断是否和第一问计算结果一致

很遗憾将这门课学成了面向已知考试题型的过拟合形式。我并不觉得我掌握了多少优化理论的知识，最多称得上知道了优化问题的大致分类和一些基本的优化应用。从我的笔记就可以看出，自第三章开始就没怎么涉及到理论的证明，确实是证不明白🤡。但这门课自下届开始被取消了hh。。。。你我皆是局内人，祝好。
