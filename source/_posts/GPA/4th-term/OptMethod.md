...
\end{aligned}
$$
本章将分别讨论约束函数是等式和约束函数含不等式的二次规划问题。
...
### 6.2 不等式约束二次规划问题 :star:

我们引入**有效集方法**。首先显然的最优解一定存在于等式约束上，那么不等式的约束仅仅在取等时有效。我们可以直接枚举每一种约束条件的组合（所有等式+枚举的不等式，并将不等式看做等式约束），然后判定当前的解是否满足没有选择的不等式约束。这种方法是可行的但是计算量极大。于是有效集方法应运而生。
...
我们定义罚函数，其中罚因子 $\sigma$：
$$
P_E(x, \alpha) = f(x) + \frac{1}{2} \sigma\sum_{i=1}^p a_i^2(x)
$$
算法流程：

![算法流程](https://dwj-oss.oss-cn-nanjing.aliyuncs.com/images/202406120820032.png)
