...
**如何准确描述一个进程？通过进程映象**。我们引入进程映象的概念。所谓进程映象就是某一时刻的进程状态，左图相对宏观，右图相对微观：

|   ![进程结构](https://dwj-oss.oss-cn-nanjing.aliyuncs.com/images/202411121040792.png)   |   ![进程映象](https://dwj-oss.oss-cn-nanjing.aliyuncs.com/images/202409270845459.png)   |
| :--: | :--: |
...
本章我们学习计算机史上“最伟大”的发明之一：并发。从五态模型来看的话，就是「运行态 $\rightleftharpoons$ 等待态」之间的转换策略。重点介绍信号量与 PV 操作。
...
详情参见：<https://blog.dwj601.cn/GPA/5th-term/ComputerOrganization/#7-4-虚存>

代码实现：https://github.com/Explorer-Dong/OS_Simulate/tree/main/Experiment_final
...
graph LR
    并发 --> 进程管理(1 进程管理)
    进程管理 -.-> 进程的存在和组织方式["`进程的存在和组织方式。PCB、进程队列`"]
    进程管理 -.-> 进程的状态与切换["`进程的状态与切换。五态模型、中断机制、原语、进程上下文`"]
    进程管理 -.-> 线程["`线程。与进程的区别`"]
    
    并发 --> 处理器调度(2 处理器调度)
    处理器调度 -.-> 产生背景["`产生背景。多进程并发，CPU计算过快`"]
    处理器调度 -.-> 调度层级["`调度层级。三级调度`"]
    处理器调度 --> 调度算法["`调度算法。缩写、各种时间、各种调度策略的特性`"]
    
    并发 --> 互斥与同步(3 互斥与同步)
    互斥与同步 --> 信号量与PV操作["`信号量与PV操作。哪些是临界区的资源（互斥锁），哪些进程有先后关系（同步锁）`"]
    
    并发 --> 死锁(4 死锁)
    死锁 -.-> 产生原因["`产生原因。本质上就是大家都拿了一部分资源，没法继续执行但又不释放已经拿到的资源`"]
    死锁 --> 银行家算法["`银行家算法。一种死锁避免策略，注意3种矩阵、试探性分配、安全序列`"]
    死锁 --> 资源分配图["`资源分配图。一种死锁解除策略，注意两类edge的定义以及如何通过删边解除死锁的`"]
```
...
graph LR
    虚拟化 --> 连续存储("连续存储")
    连续存储 -.-> 4种连续内存分配算法["`掌握4种连续内存分配算法即可`"]
    
    虚拟化 --> 分页式虚拟存储("5 分页式虚拟存储")
    分页式虚拟存储 -.-> 为什么会有虚拟存储["`为什么会有虚拟存储。还是那句话，程序的局部性`"]
    分页式虚拟存储 --> 地址转换["`地址转换。虚拟地址到物理地址的转换引发了一系列的技术问题
    1）快表机制下的地址转换
    2）多级页表下的地址转换
    3）反置页表下的地址转换
    4）缺页中断的5种页面替换算法`"]
