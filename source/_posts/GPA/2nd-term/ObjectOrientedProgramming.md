...
## 前置说明
这篇博客是博主本人学习 OOP 编程时的笔记记录，编程语言为 C++。后续会在此基础之上不断补充使用 C++ 进行 OOP 编程时的规范。
...
```
### 1.3 二叉树的应用
赠送一道算法题：[AcWing 3384. 二叉树遍历](https://www.acwing.com/problem/content/description/3387/)。
...
public:
    ...
    BinaryTree(std::string pre) {
        std::function<Node<T>*(int&)> createWithPre = [&](int& idx) -> Node<T>* {
            T data = pre[idx++];
            if (data == '#') {
                return nullptr;
            }
            Node<T>* now = new Node(data);
            now->lchild = createWithPre(idx);
            now->rchild = createWithPre(idx);
            return now;
        };
        int idx = 0;
        root = createWithPre(idx);
    }
    
    void midPrint() {
        std::function<void(Node<T>*)> midPrint = [&](Node<T>* now) -> void {
            if (!now) {
                return;
            }
            midPrint(now->lchild);
...
### 2.6 多级派生
没什么好多说的，上代码！代码看懂了，输出结果看懂了，就OK了。唯一需要强调的一点是：**在多级派生时，对于一个派生类对象的初始化，一定要附带着把他所有的“爹”也全部初始化，否则调用默认构造函数初始化他的所有“爹”**。
