...
**如果在执行中断任务时又中断了呢？这是不会发生的**。很多操作是不允许被打断的我们就需要最小操作单元，这被称作「原语」操作。例如更新进程信息（修改 PCB 中的信息）、更新进程状态（修改 PCB 所在的状态队列）、分配/回收资源。

#### 线程

**线程是什么**？线程是从进程引申出来的概念与技术。进程是资源分配和调度的最小单位，而线程是 CPU 调度的最小单位。一个进程下可以有多个线程，每一个线程共享当前进程的所有资源，线程只会有执行指令最基本的资源（PC、寄存器等）。
...
- **高级调度** 是决定哪些进程从外存对换到内存中，也就是 **分配内存** 资源。
- **中级调度** 是决定哪些进程从内存对换到外存中，也就是 **取消分配内存** 资源。
- **低级调度** 是决定哪些内存中的进程可以获得合适的 CPU 计算资源，也就是 **分配计算** 资源。
...
| 序号 | 算法             | 算法思想                                                     | 特点                                                         |
| :--: | :--------------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
|  1   | 先来先服务       | 按照作业/进程到达的先后顺序进行服务                          | 非抢占式；有利于长作业不利于短作业；不会饥饿；可用于作业/进程调度 |
|  2   | 最短作业优先     | 当前进程停止执行时，选择预计所需的 CPU 运行时间最小的进程执行 | 非抢占式；对长作业不利；会饥饿；可用于作业/进程调度          |
|  3   | 最短剩余时间优先 | 当一个新进程加入就绪队列时，它可能具有比当前运行进程更短的剩余运行时间，此时调度器将让该就绪进程抢占当前运行的进程 | 抢占式（算法 2 的抢占式版本）；对长作业不利；会饥饿；可用于作业/进程调度。 |
|  4   | 最高响应比优先   | 当前运行的进程结束或阻塞时，选择响应比最大的进程执行         | 非抢占式；结合算法 1,2(3) 的优势；不会饥饿；可用于作业/进程调度。 |
|  5   | 优先级调度算法   | 优先级调度算法根据确定的优先级来选择进程/线程，总是选择就绪队列中优先级最高者投入运行。 | 非抢占式和抢占式两种版本；适用于实时操作系统；静态优先级可能会导致饥饿；可用于作业/进程调度。 |
|  6   | 轮转调度         | 将时间切分成定长的时间片，当时间片用完后，产生时钟中断。     | 抢占式；有利于多用户、多交互式进程；时间片太大，算法会退化为先来先服务；时间片太小，进程切换频繁，开销大。不会饥饿；用于进程调度。 |
|  7   | 多级反馈队列调度 | 对各种调度算法的权衡。设置多级就绪队列，各级队列优先级从高到低，时间片从小到大。 | 长进程可能会饿死；用于进程调度。                             |
