...
1. 约束条件：从 m 个变为 n 个
    ...
    - **二元关系**从原问题中自变量的 $\ge$ 转化为 $\le$，$\le$ 转化为 $\ge$，无约束转化为 $=$
    - **线性约束**为原问题线性约束的转置
2. 自变量：从 n 个变为 m 个
    - 自变量的取值范围和原问题中约束条件的二元关系相同，$\ge$ 仍然为 $\ge$，$\le$ 仍然为 $\le$，$=$ 仍然为 $=$
...
- 最速下降法、牛顿法求解方法及其优缺点，共轭梯度法的优缺点（ch4）（直接给步长）
    ...
    至于共轭梯度法的优缺点，优点就是该法是最速下降法和牛顿法的结合，每次的搜索方向是正交的，这样就不用存储海塞矩阵并且收敛速度往往比最速下降法更快。主要用于解决正定二次函数的极小化问题。但在解决其余问题时可能会对搜索步长有极高的依赖，一旦搜索步长不够精准会导致整体的精度下降，收敛速度也会下降。
