...
对数几率回归准确来说应该叫逻辑回归，并且**不是回归**的学习任务，**而是二分类**的学习任务。

{% endnote %}

对于**二分类**任务。我们可以将线性模型的输出结果 $z=w^Tx+b$ 通过阈值函数 $g(\cdot)$ 上进行映射，然后根据映射结果 $y=g(z)$ 进行二分类。那么什么样的非线性函数可以胜任呢？

- 最简单的一种阈值函数就是**单位阶跃函数**。映射关系如下。但是有一个问题就是单位阶跃函数不是单调可微函数，因此不可取
...
- 常用的阈值函数是**对数几率函数**，也叫**逻辑函数（logistic function）**。映射关系如下：
    $$
    y = \frac{1}{1+e^{-z}}
    $$
