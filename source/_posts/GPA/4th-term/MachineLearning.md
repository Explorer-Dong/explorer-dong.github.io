...
我们希望决策树的分支结点中样本属性尽可能一致，即纯度尽可能高。

{% endnote %}

#### 4.2.1 信息增益

4.2.2 如何度量分支中样本的纯度呢？我们引入**信息熵**的概念。假设当前样本集合中含有 $t$ 个类别，第 $k$ 个类别所占样本集合比例为 $p_k$，则当前样本集合的信息熵 $\text{Ent}(D)$ 为：
$$
\text{Ent}(D) = -\sum _{k=1}^t p_k \log_2(p_k)
$$
显然，信息熵越小，分支结点的纯度越高；信息熵越高，分支结点的纯度越低。
