...
无监督图像分类任务首先在无标签图像上学习一个泛化性强的模型 (encoder)，然后在具体的图像分类任务上使用有标签图像进行微调 (decoder)，可以简单理解为 **学习一个很厉害的 encoder**。工作范式如下图所示：

![无监督学习的工作范式](https://cdn.dwj601.cn/images/20250401080834825.png)

/// fc
无监督学习的工作范式，上半部分为训练 encoder，下半部分为训练 decoder
///

### 自监督学习

自监督学习 (Self-supervised learning) 利用图像本身的信息学习有效的视觉表示。介绍两种方法。

**1）掩码自编码器**。通过最小化重构损失，让模型学习到图像的特征。如下图所示：

![自编码器工作范式](https://cdn.dwj601.cn/images/20250401101812602.png)

/// fc
Masked Auto Encoder 工作范式，[Masked Autoencoders Are Scalable Vision Learners - CVPR 2022](https://openaccess.thecvf.com/content/CVPR2022/papers/He_Masked_Autoencoders_Are_Scalable_Vision_Learners_CVPR_2022_paper.pdf)
///

**2）自定义低级语义标签**。注意：现在我们的任务是训练一个能很好提取图像特征的 encoder，因此这里的标签不是图像语义中的具体类别，而是诸如「旋转角度、子块顺序」等低级语义的类别。如下两图所示：

| ![以「旋转角度、随机颜色、随机反转、剪裁」作为新标签](https://cdn.dwj601.cn/images/20250401095117995.png) | ![以「子块顺序」作为新标签](https://cdn.dwj601.cn/images/20250401095121127.png) |
| :----------------------------------------------------------: | :----------------------------------------------------------: |
|                  以「旋转角度」等作为新标签                  |                   以「子块顺序」作为新标签                   |

### 对比学习

对比学习 (Contrastive Loss) 认为一张图像的不同视角应该具有相似的语义表示，而不同的图像大概率具有不同的语义表示。同样介绍两种方法。

1）**SimLCR**。该算法的思想就是对数据做增强，让模型「最小化同一张」同时「」。如下两图所示：



2）**Zero-shot Learning**。也就是所谓的零样本学习，虽然我觉得这么翻译并不合适。该方法旨在让模型可以预测出训练数据之外的图像类别。当然这不是让模型造词，完整的类别 S 是有的，只不过训练集中的类别 T 是 S 的子集。
