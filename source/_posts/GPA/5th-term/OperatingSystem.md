...
在类 Unix 操作系统中，文件由「**索引结点**」和「**文件数据**」两部分组成。具体的，索引结点 inode 用来存储「文件的元数据」和「文件数据所在的物理存储块」，而文件数据就是文件的原始信息。可以简单的理解为如下的结构体：
...
大量的文件需要我们进行合理的管理，以树形文件目录为例。为了 **便于管理**，设计者将每一个 inode 进行唯一性编号（记作 inode_id），这样用户打开一个文件时，操作系统会根据 inode_id 快速查询到对应的 inode 并基于此 inode 将对应的文件加载到内存。

写到这里我不禁疑惑：所以和树形目录结构有什么关系吗？是的，从开发者角度来说，树形目录并没有任何作用，他们只需要关心什么 inode_id，别的都不操心。但是从用户角度来说，树形目录的意义就很大了。具体的，对于用户来说，树形目录可以让我们分级分类管理多而杂的文件，并且在不同的树路径下，我们可以取相同名称的文件。与此同时，不同的子树存储的数据可以被划分到不同的硬件存储器中，这就方便了用户从物理意义上将数据进行迁移。下面展示一个可重名的树形文件目录在不同视角下存储的信息：
