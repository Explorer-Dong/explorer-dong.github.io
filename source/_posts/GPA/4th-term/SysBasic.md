...
$$
\begin{aligned}
\text{源代码 $\xrightarrow[]{\text{①预处理}}$ 源代码 $\xrightarrow[]{\text{②编译}}$ 汇编代码 $\xrightarrow[]{\text{③汇编}}$ 机器代码 $\xrightarrow[]{\text{④链接}}$ 机器代码}
\end{aligned}
$$
...
本章我们要学的「指令集」其实就是 **CPU 运作机理的一个重要部分**。它定义了 CPU 理解和执行指令的规则和方法，也是程序员和编译器与 CPU 交互的接口。
...
正如上面所言，机器指令本质上就是一个人为定义的含有某些意义的 01 序列，查询相应的表可以得知对应的汇编语言格式。不需要记忆多少东西，遇到不懂的就直接查表即可。

举个例子。下方表格展示了一个经典的转换，怎么转换不重要，重要的是能理解对应汇编语言的逻辑：

| 机器语言（转换为 16 进制） |  汇编语言（AT&T 格式）  |
| :------------------------: | :---------------------: |
...
R 表示取寄存器操作数，M 表示取存储器操作数：

值运算语句：`mov 0x8(%ebp,%edx,4), %eax` 功能为 `R[eax] = M[R[ebp] + R[edx] * 4 + 8]`

地址运算语句：`lea 0x8(%ebp,%edx,4), %eax` 功能为 `R[eax] = R[ebp] + R[edx] * 4 + 8`

- 第一条指令表示：将计算出来的地址 `R[ebp] + R[edx] * 4 + 8` 对应的值 `M[R[ebp] + R[edx] * 4 + 8]` 赋给寄存器 `%eax`，此时寄存器中存储的信息为**值** `R[eax]`
- 第二条指令表示：将计算出来的地址 `R[ebp] + R[edx] * 4 + 8` 赋给寄存器 `%eax`，此时寄存器中存储的信息为**地址** `R[eax]`

#### 3.3.2 定点算数运算指令

加减乘除分别为：`add, sub, mul, div`，都是将「源目的地址」的值运算到「转移目标地址」，即左边对应的值运算到右边对应的值。

#### 3.3.3 按位运算指令

常用的有按位运算，以及算数、逻辑的左移和右移。按位运算中有一个用于判定单个数情况的指令为 `test`，`test` 不修改右边对应的值，其余都遵循上述「左运算到右进而修改右」的规则。

#### 3.3.4 控制转移指令

常用的有无条件转移，对应 C 语言中的 `goto` 关键字，汇编表示为 `jmp` 等。以及有条件转移，比如 `je, jne` 等，取决于逻辑运算的结果。
...
缓冲区溢出其实和越界访问是一个东西，只不过缓冲区溢出是单独针对栈空间中对局部数组进行了越界访问。一般而言的越界访问往往表示错误访问了栈空间中的返回地址导致程序出现意想不到的错误。

## 第4章 程序的链接 TODO

{% note light %}

在编写大型程序时，为了**提升效率**以及**不断复用**，往往需要模块化开发，这就需要我们对多个分散的源代码片段进行整合。目前的整合方法是：首先对每一个源代码片段 $\text{(*.c)}$ 分开进行编译、汇编生成对应的可重定位目标代码文件 $\text{(*.o 或 *.obj)}$，然后通过链接器 $\text{(linker)}$ 将所有的可重定位目标代码文件与使用到的其他库的可重定位目标代码文件结合，形成最终的可执行文件 $\text{(*.exe)}$。本章主要讨论链接器的工作原理。
...
- 模块化
    - 一个程序可以分成很多源程序文件，便于管理
    - 可构建公共函数库，如数学库，标准 I/O 库等
- 效率高
    - 时间优，由于可以可以分开编译因此只需重新编译被修改的源程序文件，然后重新链接即可
    - 空间优，源文件中无需包含共享库函数的源码因此只需包含所调用函数的代码即可

### 4.2 目标文件格式 TODO

#### 4.2.1 ELF 目标文件格式

包含两种视图对应 4.2.2 中的 $\text{(*.o 或 *.obj)}$ 文件和 4.2.3 中的 $\text{(*.exe)}$ 文件。
