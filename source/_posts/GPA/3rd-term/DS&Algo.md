...
一般来说，红黑树是一个较为通用的选择，而在需要严格平衡性时，AVL 树可能是更好的选择。有关平衡树的知识点较为复杂，将会在 [进阶数据结构](#进阶数据结构) 部分详细展开。
...
考虑到此处是对数据结构的初步介绍，为了简明扼要，图的相关算法就在 [图论](#图论) 部分中再详细介绍。
...
**快速排序**。这是一种不稳定的排序算法，核心思想是 [分治](#分治)。具体地，以升序为例，如果一个序列是有序的，那么对于该序列中的每一个元素，其左边所有元素都应该比右边所有元素小。基于该先验，我们就有了快速排序算法：
