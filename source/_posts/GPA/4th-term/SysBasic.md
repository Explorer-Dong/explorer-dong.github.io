...
ISA 定义了机器语言级虚拟机的所有功能。再大白话一点就是一套**操控机器的规则**。
...
2. 寄存器寻址：指定寄存器 R 的内容为操作数。在别的寄存器中找数据；
...
`mov 0x8(%ebp,%edx,4), %eax` 功能为 `R[eax] = M[R[ebp] + R[edx] * 4 + 8]`

`lea 0x8(%ebp,%edx,4), %eax` 功能为 `R[eax] = R[ebp] + R[edx] * 4 + 8`

上述例子中，R 表示取寄存器操作数，M 表示取存储器操作数。可以将 lea 看做 mov 的特殊，即 C 语言中的取地址操作。

#### 3.3.2 定点算数运算指令

加减乘除分别为：`add, sub, mul, div`

关于定点寄存器的说明：`EAX,EBX,ECX,EDX` 均为 32 位寄存器，对应的 `AH,BH,CH,DH` 均为低16 位寄存器，`AL,BL,CL,DL` 均为低 8 位寄存器
