...
$$
{% fold info @几何意义：F(x,y) 即左下方无界矩形的面积 %}
...
本章我们将学习随机变量的一些数字特征。所谓的数字特征其实就是随机变量分布的一些内在属性，比如均值、方差、协方差等等，有些分布特性甚至可以通过某个数字特征而直接觉得，接下来开始介绍。
...
- 离散型
    ...
    s.t.\quad &P(x_i)=p_i \quad (i=1,2,\cdots)
    \end{aligned}
    $$
