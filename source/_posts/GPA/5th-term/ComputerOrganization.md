...
既然叫做虚存，那么每一个进程中的每一条指令对应的地址就都是虚拟地址，然而 CPU 访问数据时需要能够直接对应到实际存储单元的物理地址，因此虚拟化技术一个最重要的部分就是如何将虚拟地址（逻辑地址）转化为物理地址（实际地址）。如下图所示，展示了使用虚拟化技术的 CPU 工作逻辑：
...
这里再插一嘴。虚拟化技术对于程序空间要求高，但是计算速度要求不是很严格的应用场景下（例如服务器、台式机、笔记本），可谓是神来之笔，大大提升了程序存储开销的上限。但通过上面介绍的虚存工作逻辑也不难发现，为了实现虚拟化技术，我们不得不设计一个 MMU 来满足需求，这就会导致一定的时间开销以及设计架构的复杂化。没有一种技术可以一以贯之，虚拟化技术也是如此。对于程序空间要求不大，但是运算速度和延迟要求很严格的场景下（例如嵌入式），虚拟化技术就不能使用了，此时 CPU 访问的指令就直接对应了 cache 或内存中的实际物理地址。

##### 7.4.3 虚存实现

在虚拟化的背景下，每一个程序/进程都对应了一个统一大小的虚拟存储空间。但对于运行在内存中的进程而言，由于机器的内存空间大小 $\ll$ 进程的虚存空间大小。设计者将虚存中多出来的空间分配到外存中，内存中仅仅保存虚存中的活跃部分，而内存中保存的活跃部分中又有更活跃的部分，于是设计者就将这段更活跃的部分放到缓存中。具体的内存与虚存的对应关系有三种：分页式、分段式、段页式。在下面的「虚存实现」中将会详细讲解。

在知道了。进程的虚拟空间中存储的数据/指令都对应一个地址，这个地址叫做虚拟地址（逻辑地址），CPU 需要的是实际地址（物理地址）
