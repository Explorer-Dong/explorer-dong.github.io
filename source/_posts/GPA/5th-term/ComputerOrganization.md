...
三、最近最少使用 (Least Recently Used, LRU)
...
四、最不经常使用 (Least Frequently Used, LFU)
...
在「内存的空间大小不一且远小于现代程序的空间需求」以及「现代 OS 都支持并发程序」的背景下，虚拟存储空间技术应运而生。需要注意的是，虚拟存储空间并不对应计算机中的实际物理器件，而是一种 **设计理念**。通过虚拟化技术，不仅仅提升了程序的可使用空间大小，也可以让开发者不受限于不同主机内存大小不一的问题，而只需要关心每一个程序对应的运行逻辑，更进一步的，虚拟化技术也可以保护进程的数据安全，确保程序并发执行时不会出现访问异常地址等错误。

##### 虚存的工作逻辑

既然叫做虚存，那么每一个进程中的每一条指令对应的地址就都是虚拟地址，然而 CPU 访问数据时需要能够直接对应到实际存储单元的物理地址，因此虚拟化技术一个重要的部分就是如何将逻辑地址（虚拟地址）转化为物理地址（实际地址）。如下图所示，展示了使用虚拟化技术 CPU 的访存逻辑：

![使用虚拟化技术的 CPU 的访存逻辑 | 简化版](https://dwj-oss.oss-cn-nanjing.aliyuncs.com/images/202410301119466.png)

如上图所示。CPU 在拿到内存中某个进程的某条指令中的虚拟地址 VA 后，首先需要通过存储器管理单元 (Memory Management Unit, MMU) 将其转化为物理地址 PA，再根据物理地址进行诸如 cache $\to$ 内存 $\to$ 外存的数据访存逻辑。
...
有了上述的虚存工作概念，我们尝试实现虚拟化技术。由于进程的虚拟空间 $\gg$ 机器的内存空间，我们不可能把一个完整的进程一次性全部加载进内存，因此虚存最重要的一个设计理念就是将部分进程加载到内存，其余部分仍然保持在外存中。

这种设计理念似乎似曾相识？是的，缓存和内存不也是这样设计的吗！既然如此，理解起来似乎又容易了一些。同样的我们需要解决诸如分块、映射、替换、读写一致性等问题。围绕这些问题，目前的虚存实现主要分为「分页式」、「分段式」和「段页式」三种。下面主要讲解 **分页式** 虚拟化技术。我将会从 **分块**、**映射**、**替换与读写一致性**、**快表** 和 **页目录** 五个角度展开。

1）分块方式

对于分页式虚存，设计者将内存和进程分别划分为等空间大小的子模块。其中内存中的子模块一般叫做实页（OS 中叫「页框」），进程中的子模块一般叫做虚页（OS 中叫「页」）。

2）映射策略

对于分页式虚存，进程和内存之间是通过全相联映射实现的，即进程中任意一个虚页可以对换到内存中任意一个实页中。

值得注意的是，这里的对换与内存和 cache 的对换并不是一个意思。在内存和 cache 中，存储的都是实际的物理地址，因此内存和 cache 的对换就是把所有的信息全部对换并通过一个标记位来表示是否有效。但是在外存虚页和内存实页的对换中，由于进程的虚页地址和内存的实页地址完全不同，因此我们不能像内存和 cache 一样把所有信息进行对换，于是 **页表** 应运而生。

![](https://dwj-oss.oss-cn-nanjing.aliyuncs.com/images/202411121121875.png)

页表结构解读。如上图所示，给出了页表的示意结构。通过页表，我们建立起了「进程的虚页」与「内存的实页」之间的对应关系。在页表中每一行数据被称为页表项，对应一个虚页的映射信息。例如上图所示一共有 8 个虚页，那么页表就一共有 8 个页表项。为了节省空间，每一个进程只会在其 PCB 中存储对应页表的首地址。

地址转换逻辑。我们知道，CPU 在访问某一个虚拟地址时，需要利用 MMU 将该虚拟地址转换为物理地址才能访问到对应的信息。具体的，首先根据虚拟地址中的高位字段得到虚页号，然后根据虚页号定位到页表中的一个页表项：
...
关于页面替换算法。和缓存的替换算法完全一致，这里不再赘述。

关于读写的一致性。页表中有一项叫做修改位。如果在读写或被替换时发现修改位为 1，则表明对应的页数据被修改了，此时一般采用 write back 策略来保持一致性。

4）快表策略

由于每一个进程的页表都会被加载到内存中，这就意味着每次做地址转换访问页表时都需要访问内存，那为什么不能访问 cache 呢？设计者将经常访问到的页表项拿出来放到 cache 中组成了新的页表叫做后备转换缓冲器 (Translation Lookaside Buffer, TLB)，通常称其为快表。这样一来在地址转换时就会首先查询 TLB，如果没找到就再去访问主存中的页表（在快表的概念下主存的页表就被称为慢表）。
...
至此，分页式虚存就全部介绍完毕。下图展示了在一个具有 cache 机制和虚拟化技术的系统中 CPU 访存一次的完整逻辑：
