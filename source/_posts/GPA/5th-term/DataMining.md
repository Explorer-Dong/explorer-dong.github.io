...
这是一个 **无监督** 集成树模型。对于森林中的每一棵树，初始时含有所有的数据对象，每次对树的所有结点进行分裂，分裂规则为选定某一个属性并计算出阈值，然后基于这个阈值将结点中的数据对象划分为两部分，直到一个结点只含有一个数据对象或者达到最大深度时停止分裂。孤立森林中树之间的区别本质上就是划分准则选择的属性顺序不同。这与决策树算法比较类似。
...
上述库作者写了一篇论文，测试了 30 种异常检测算法在 57 种基准数据集上的检测性能：

- [ADBench: Anomaly Detection Benchmark](https://arxiv.org/pdf/2206.09426)

中文技术博客：

- [异常检测算法库PyOD](https://blog.csdn.net/qq_20144897/article/details/125477691)
- [一文详解8种异常检测算法（附Python代码）](https://blog.csdn.net/m0_59596937/article/details/128877355)
