...
- 选择 `6 * 2'`
    - 严格凸函数的定义（判断海塞矩阵是否正定即可）
    - 凸规划问题的定义和性质
    - 求二元函数的极值（先利用一阶必要条件求驻点，然后利用海塞矩阵的正定性判定极值点。特征值之和为海塞矩阵的迹，特征值之积为海塞矩阵的行列式，若海塞矩阵正定则对应极小，负定对应极大）。答案：一个极大一个极小，另两个什么都不是
    - 拉格朗日函数中等式乘子的性质（等式对应的乘子 $\lambda>0$​）
    - 海塞矩阵正定负定和极值的关系（显然的）
...
- 证明 `1 * 10'`
    - 证明某个优化问题是凸规划问题，注意是高维的（证明可行域是凸集、目标函数是凸函数即可）
