...
考点：给定一条指令，能够知道控制器中会产生哪些控制信号，以及此时指令中的数据在数据通路中是如何流通的。
...
单周期处理器的设计思路可以从指令的执行过程来理解。我们知道：一个 CPU 能够不断的读取内存中的指令，并且可以解析这些指令生成准确的控制信号，基于这些控制信号配合合理的线路，在通电后就可以计算出正确的结果。（显然处理器的时钟周期需要设定为耗时最长的指令执行时间）
...
本章我们学习「**流水线处理器**」的设计。与上述「单周期处理器」类似，学习 11 条 MIPS 指令的数据通路和控制器的设计方法。

#### 6.1 流水线处理器的设计 :star:

所谓流水线，其实就是并行执行指令。首先我们需要将指令统一划分为相同的字段，例如（取指、译码、执行、访存、写回）五个字段。接下来就可以并行执行指令了，当然这里的并行并不是很多指令一起执行，这样显然就不能正确运行程序逻辑了，所谓并行指的是指令仍然顺序执行，只不过可以分字段并行，例如：

- **第一条指令**：会先进入 **取指 (Ifetch)** 阶段，然后依次进入 **译码 (Reg/Dec)**、**执行 (Exec)**、**访存 (Mem)**、**写回 (Write)** 阶段。
- **第二条指令**：在第一条指令进入译码阶段时，第二条指令可以开始进入取指阶段，然后依次进入其他阶段。
...
但并不是所有的指令都像 Load 指令一样具有所有的段，为了能够让所有的指令能够在统一的段划分下并行执行，需要对其余的指令进行空段扩展，使其均成为五段指令。

#### 6.2 冒险情况解决策略

**结构冒险 (hardware resource confilicts)** :star:
...
2. 将 IM 和 DM 分离。由于流水线策略的并行机制，取指令 (Ifetch) 和写存储器 (Mem) 操作仍然会因为并行而同时访问存储器，因此我们可以将指令存储器 (Instruction Memory, 简称 IM) 和数据存储器 (Data Memory, 简称 DM) 分离；
3. 将寄存器的读口和写口分离。同样由于流水线策略的并行机制，会导致指令写寄存器 (Write) 和指令读寄存器 (Reg/Dec) 的情况同时发生，为了解决这个问题，可以将寄存器的读口和写口分离。

示意图如下所示：

![结构冒险解决策略](https://dwj-oss.oss-cn-nanjing.aliyuncs.com/images/202412040949539.png)

**数据冒险 (data dependencies) ** :star:

现象：写后读。

解决：两种技术：

1. 转发技术 (Forwading)。现在寄存器的值不仅仅取决于指令指向的寄存器的数据，也有可能是之前的指令运算后的结果。在物理实现上就是前者属于从前面的段寄存器传过来的数据，后者属于从后面的段寄存器传过来的数据；
...
特殊的。上述示意图中首先执行的是算数运算，这样就可以将 ALU 的运算结果直接传递给后续指令的取数结果。但如果首先执行的是 Load 指令，那么运算结果是在 Dm 之后才能从存储器中读出来，由于「后续指令只能依赖已经更新的数据」，因此这里会出现问题。如下图所示：

![Load-use 数据冒险](https://dwj-oss.oss-cn-nanjing.aliyuncs.com/images/202412041143660.png)

解决方法就是在 Load 指令的后面增加一个时钟周期的阻塞，然后再转发。至于怎么增加，可以从硬件、软件、编译三个角度进行：

1. 硬件角度。阻塞一个周期；
2. 软件角度。插入一条无关指令；
3. 编译角度。将编译的指令中无关的指令提前到 Load-use 指令之间。

**控制冒险 (changes in program flow)**
...
不难发现，在上述三种映射策略中，除了模映射是一一对应直接替换，另外两种映射策略由于存在随机性，需要考虑替换哪一行。我们介绍最佳时间 (OPT)、先进先出 (FIFO)、最近最少使用 (LRU)、最不经常使用 (LFU)、时钟策略 (Clock) 五种替换算法。

**最佳时间 (Optimism Time, 简称 OPT)**
...
**最不经常使用 (Least Frequently Used, 简称 LFU)**
...
注意：这里的对换与内存和 cache 的对换并不是一个意思。在内存和 cache 中，存储的都是实际的物理地址，因此内存和 cache 的对换就是把所有的信息全部对换并通过一个标记位来表示是否有效。但是在外存虚页和内存实页的对换中，由于进程的虚页地址和内存的实页地址完全不同，因此我们不能像内存和 cache 一样把所有信息进行对换，于是 **页表** 应运而生。
...
关于页面替换算法。和缓存的替换算法完全一致，见 chapter7.1.4。

关于读写一致性。页表中有一项叫做修改位。如果在读写或被替换时发现修改位为 1，则表明对应的页数据被修改了，此时一般采用 write back 策略来保持一致性。
...
主要有：输入输出设备、外部存储设备、网络设备三类设备。其中外部存储设备就是第七章存储器中的外存部分，这里主要介绍磁盘。

#### 8.2 I/O 接口

一般而言，设备通过接口与南桥芯片相连，主板中的数据通过名为总线的线路进行数据传输。

#### 8.3 I/O 传输

设备与内存的数据传输有三种方式：

1. OS 主动轮询。操作系统定时从设备读取数据；
2. 设备主动请求。设备主动向 OS 发送数据传送请求，OS 通过中断并执行中断程序来与设备进行数据传输；
3. 通过 (Direct Memory Access, DMA) 控制器一次性传输大批量数据。设备向 DMA 控制器发送数据传送请求，DMA 再向 CPU 发送「让出总线」的请求，CPU 响应后设备即可与内存进行大批量的数据传输。这一步所谓的不占用 CPU，其实是使用一个「精简 CPU」来代替其 I/O 的功能。这里的精简 CPU 就是 DMA 控制器。
