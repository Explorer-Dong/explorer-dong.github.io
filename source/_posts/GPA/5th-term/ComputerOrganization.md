...
**对于指令的运算需求，如何设计出对应的硬件来支持呢？** CPU 中的 ALU 单元就是负责运算的部件。其有两个输入运算数 A 和 B 以及一个逻辑控制输入 ALUctr。ALU 内部的逻辑结构如下图所示：
...
- 若是条件转移。就将 rs 和 rt 进行运算后，根据运算结果决定是否要转移到相应的地址处执行，具体的寻址方式是相对寻址，即将 PC 加上立即数对应的偏移量得到有效地址。
...
- 主要是无条件转移指令。只需要将 PC 的高四位拼接上 26 位直接地址再在末尾补两个 0 即可得到跳转地址。
...
注：CPU 通过寄存器中的信息进行计算的大致流程见 https://chatgpt.com/share/6767878b-702c-800a-99eb-d146406b24e7
...
**结构冒险 (hardware resource confilicts)**
