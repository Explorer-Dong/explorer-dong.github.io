...
主要参考西瓜书 [《机器学习与模式识别》· 周志华 · 清华大学出版社](https://github.com/jingyuexing/Ebook/blob/master/Machine_Learning/%E6%9C%BA%E5%99%A8%E5%AD%A6%E4%B9%A0_%E5%91%A8%E5%BF%97%E5%8D%8E.pdf) 和 南瓜书 [《机器学习》（西瓜书）公式详解](https://github.com/datawhalechina/pumpkin-book)。
...
1. 一般式：
    $$
    ||\mathbf{x}||_p^k = \left ( \left ( \sum_{i = 1}^{m}|x_i|^{p} \right)^{\frac{1}{p}}  \right)^k
    $$
...
3. L2 正则化：
    $$
    \begin{aligned}
    ||\mathbf{x}||_2^2 &= \left ( \left ( \sum_{i = 1}^{m}|x_i|^{2} \right)^{\frac{1}{2}}  \right)^2 \\
    &= \sum_{i = 1}^{m}|x_i|^{2}
    \end{aligned}
    $$
...
$$
R^2 = 1 - \frac{\sum_{i = 1}^m(f(x_i)-y_i)^2}{\sum_{i = 1}^m(\bar{y} - y_i)^2},\quad \bar{y} = \frac{1}{m}\sum_{i = 1}^m y_i
$$
...
![混淆矩阵](https://dwj-oss.oss-cn-nanjing.aliyuncs.com/images/202403121018205.png)
...
&\textbf{w} \in R^d, \textbf{x} \in R^d, b \in R
\end{aligned}
$$
我们将从回归和分类两个任务展开参数 $\textbf{w}$ 和 $b$ 的学习策略：
...
对于线性回归 **模型** $f(\bf{x}) = \bf{w}^T \bf{x} + \bf{b}$，如何学习参数 $w$ 和 $b$ 使得预测值 $f(x)$ 与真实值 $y$ 尽可能的接近？我们引入 **学习准则**：最小化均方误差，也就是所谓的最小二乘估计。直观的理解就是寻找一条直线使得所有的样本点到该直线的距离之和尽可能的小。至于 **优化算法**，使用梯度下降即可。

#### 3.1.1 一元线性回归

现在假设只有一个属性 x，对应一维输出 y。我们试图根据已知的 \< x, y \> 样本数据学习出一个模型 $f(x_i) = wx_i+b$ 使得尽可能准确的预测未来的数据。那么此时如何求解模型中当目标函数取最小值时的参数 w 和 b 呢？很显然我们可以使用无约束优化问题的一阶必要条件求解。
...
现在我们保持输出不变，即 $y$ 仍然是一维，将输入样本的特征从 $1$ 维扩展到 $d$ 维。如何学习模型中的参数 $\textbf{w} = (w_1,w_2, \cdots , w_d)^T$ 和 $b$ 呢？

方法和一元线性回归是一样的。开始之前我们采用向量的方式表示一下数据：$X_{m\times(d+1)}$ 为输入的样本特征矩阵，$\hat w$ 为参数向量，$y$ 为样本标记值，$f(x)$ 为模型：
$$
X = 
\begin{bmatrix}
...
对于 **二分类** 任务。我们可以将线性模型的输出结果 $z=w^Tx+b$ 通过阈值函数 $g(\cdot)$ 进行映射，然后根据映射结果 $y=g(z)$ 便可以进行二分类。那么什么样的非线性函数可以胜任阈值函数一职呢？
...
\end{cases}
$$
常用的一种阈值函数是 **对数几率函数**，也叫 **逻辑函数（$\text{logistic function}$）**。映射关系如下：
$$
y = g(z) =  \frac{1}{1+e^{-z}}
$$
...
现在我们站在前人的肩膀上学习到了一种阈值函数：逻辑函数（$\text{logistic function}$）。在开始讲解参数 $w$ 和 $b$ 的学习过程之前，我们先解决一个疑问：为什么英文名是 $\text{logistic}$​，中文翻译却成了 **对数几率** 函数呢？
...
以「$\kappa$-统计量」为例进行说明。两个学习器的卡帕结果越接近 1，则越相似。可以分析出：
...
![k 均值算法伪代码](https://dwj-oss.oss-cn-nanjing.aliyuncs.com/images/202407021942941.png)
...
近邻成分分析 $\text{(Neighborhood Component Analysis, 简称 NCA)}$，目标函数是：最小化所有数据点的对数似然函数的负值。

#### 10.6.2 LMNN

大间隔最近邻 $\text{(Large Margin Nearest Neighbor, 简称 LMNN)}$，目标函数是：最小化同一个类别中最近邻点的距离，同时最大化不同类别中最近邻点的距离。
