...
- 实验文档 - 清华大学计算机组成原理实验文档：https://lab.cs.tsinghua.edu.cn/cod-lab-docs/labs/1-overview/
- 路人博客 - verilog 入门：https://blog.csdn.net/qq_42622433/article/details/126423119
...
注：指令系统即指令集体系结构 (Instruction Set Architecture, 简称 ISA)。

在开始学习指令系统之前有必要知道一些关键术语：

- 在 **逻辑层面** 上。CPU 能理解的只有 **指令**，不同功能的指令组成了一个集合叫做 **指令集**，指令集中不同的指令的组合方式就构成了 **指令集体系结构**，基于特定的指令集体系结构开发的 **应用程序** 可以运行在任何一个支持相同 ISA 的硬件上 （也就是所谓的兼容）；
...
本章我们就会学习上述指令集体系结构中的 MIPS 指令系统。将会分为「指令系统的设计」和「程序的机器级表示」两个部分展开。

#### 4.1 指令系统的设计

一条指令必须包含的信息：操作码、源操作数地址、保存地址、下一条指令地址。

#### 4.2 程序的机器级表示

本目我们主要学习高级语言（C 语言）到机器指令（MIPS 指令集）的转换。在计算机系统基础这门课程中已经学习了 C 语言与 IA-32 指令之间的转换，见 [<https://blog.dwj601.cn/GPA/4th-term/SysBasic/#第3章-程序的转换及机器级表示>](https://blog.dwj601.cn/GPA/4th-term/SysBasic/#第3章-程序的转换及机器级表示)。很多基础性的概念诸如「程序转换步骤」就不再赘述。主要学会「MIPS 架构下机器代码的汇编表示」即可。

汇编代码的规范格式有很多种，这往往取决于指令集，在计算机系统基础中针对 IA-32 指令集我们采用的是 AT&T 格式的汇编。在 MIPS 架构下我们采用 MIPS 独有的汇编格式。下面给出 MIPS 汇编下三大类指令的表示：
...
在内存的空间大小远远小于现代程序的空间需求，以及现代 OS 都支持并发程序的背景下，「虚拟存储空间」技术应运而生。需要注意的是，虚拟存储空间并不对应计算机中的实际物理器件，而是一种 **设计理念**。通过虚拟化技术，1）不仅仅提升了程序的可使用空间大小，2）虚存中的虚拟地址也可以让开发者不用关心程序在内存中运行时对应的物理地址，只需要关心每一个程序对应的运行逻辑，3）更进一步的，虚拟化技术也可以保护进程的数据安全，确保并发执行程序时不会出现访问异常地址等错误。
...
![使用虚拟化技术的 CPU 的访存逻辑](https://dwj-oss.oss-cn-nanjing.aliyuncs.com/images/202410301119466.png)

如上图所示。CPU 在拿到内存中某个进程的某条指令中的虚拟地址 VA 后，首先需要通过存储器管理部件 (memory management unit, 简称 MMU) 将其转化为物理地址 PA，再根据物理地址进行诸如 cache $\to$ 内存 $\to$ 外存的数据访存逻辑。而接下来的虚存实现部分，本质上就是在讲解 MMU 的实现策略。

这里再插一嘴。虚拟化技术对于程序空间要求高，但是计算速度要求不是很严格的应用场景（例如服务器、台式机、笔记本）可谓是神来之笔，大大提升了程序存储开销的上限。但通过上面介绍的虚存工作逻辑也不难发现，为了实现虚拟化技术，我们不得不设计一个 MMU 来满足需求，这就会导致一定的时间开销以及设计架构的复杂化。没有一种技术可以一以贯之，虚拟化技术也是如此。对于程序空间要求不大，但是运算速度和延迟要求很严格的场景（例如嵌入式），虚拟化技术就不能使用了，此时 CPU 访问的指令就直接对应了 cache 或内存中的物理地址。

##### 7.4.3 虚存实现

在虚拟化技术的背景下，每一个程序/进程都对应了一个 **统一大小** 的虚拟存储空间。

由于进程的虚存空间 $\gg$ 机器的内存空间，设计者将虚存中多出来的空间分配到外存中，内存中仅仅保存虚存中的活跃部分，而内存中保存的活跃部分中又有更活跃的部分，于是设计者就将这段更活跃的部分放到缓存中。具体的内存与虚存的对应关系有三种：分页式、分段式、段页式。下面主要讲解分页式虚拟化技术，分段式和段页式简单介绍。
